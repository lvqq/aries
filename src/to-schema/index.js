...
const getSchemaItem = (params) => {
  const {
    type, description, required, errorMessage,
  } = params;
  let retVal = {
    type,
  ...
  if (type === 'string') {
    const {
      minLength, maxLength, pattern, 'x-format': format = {},
    } = params;
    retVal = {
      ...retVal,
      ...minLength ? { minLength } : {},
    ...
    const { properties = {}, required: objectRequired = [] } = params;
    retVal.properties = {};
    Object.keys(properties).forEach((key) => {
      retVal.properties[key] = {
        ...getSchemaItem(properties[key]),
        required: objectRequired.includes(key),
      };
    });
  } else if (type === 'array') {
...
module.exports = async ({
  ...
  const manifestObj = {
    ...
    manifest: schemas.map((item) => omit(item, ['schema'])),
  };
  return [
    ...files,
    {
